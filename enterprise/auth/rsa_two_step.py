...
class RsaTwoStepAuthProvider(AuthProvider):
    ...
    def authenticate(self, headers: Dict[str, str]) -> Dict[str, str]:
        if not self.priv_key or not self.pub_key:
            ...
            print("[rsa_two_step auth] 已自动生成 RSA 密钥对", file=sys.stderr)
        
        # Step 1: 注册（发送 APP_ID + 公钥，获取临时凭证 + 服务端公钥）
        ...
        if not reg_result.get("status"):
            raise RuntimeError(f"注册失败: {reg_result.get('errmsg', reg_result.get('msg', '未知错误'))}")
        secrit, spk = reg_result["secrit"], reg_result["spk"]
        self._spk = spk
        
        # Step 2: 用服务端公钥加密临时凭证，换取 token
        ...
        if not token_result.get("status"):
            raise RuntimeError(f"获取 Token 失败: {token_result.get('errmsg', token_result.get('msg', '未知错误'))}")
